...
Ks=round(1024.*[0.2, 0.4,0.6,0.8]);
% Ks=round(1024.*[0.2, 0.25]);
N_K=length(Ks);
...
D=dctmtx(h);
method="ISTA";
[I_Recovered_ISTA,I_Recovered_ISTA_complete]=doExp(I,D,h,w,Ks,method);
...
function [s_hat,H_hat,idices] = ISTA(y,H,sigma_n)
[M,N]=size(H);
maxIter=N;
iterflag=1;
epsilon=1e-4;
...
function [I_Recovered,I_Recovered_complete]=doExp(I,D,h,w,Ks,method)
[H,W]=size(I);
sigma_n=sqrt(1e-3);
N_K=length(Ks);
h_group=H/h; w_group=W/w;
...
for k=1:N_K
    ...
    parfor iter=1:h_group*w_group
        ...
        if method=="OMP"
            ...
            [J_hat_vec_par{iter},~,~] = ISTA(y,H,sigma_n)
        end
